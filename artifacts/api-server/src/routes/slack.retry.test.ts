    ...
    db: {
      ...
      update: (table: object) => ({
        set: (values: Record<string, unknown>) => {
          ...
          // Reflect the write back onto the row, as Postgres would — the
          // approval gate reads the persisted flag after recordApproval writes it.
      ...
      insert: (table: object) => ({
        values: (values: Record<string, unknown>) => {
          ...
          const key =
            table === replySendsTable
              ? String(values.sendKey)
              : String(values.idempotencyKey ?? `${(table as { _name: string })._name}:auto`);
      ...
      delete: (table: object) => ({
        where: (condition: unknown) => {
          ...
          else mocks.dbDeleteWheres.push(condition);
